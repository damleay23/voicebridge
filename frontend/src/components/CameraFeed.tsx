...
export default function CameraFeed({ children }: CameraFeedProps) {
  ...
  return (
    <div className="relative w-full h-full bg-black rounded-2xl md:rounded-[32px] overflow-hidden flex items-center justify-center">
      ...
      {cameraActive && !handDetected && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
          <span className="text-slate-600 text-xs md:text-sm text-center px-4">
            {wsConnected ? 'Show your hand to the camera' : 'Loading AI model...'}
          </span>
        </div>
      )}

      {/* HUD Corners — responsive size */}
      <div className="absolute top-[20%] left-[20%] right-[20%] bottom-[20%] pointer-events-none z-10">
      ...
      {/* Bottom controls */}
      <div className="absolute bottom-3 left-3 md:bottom-6 md:left-6 z-30 flex items-center space-x-2">
